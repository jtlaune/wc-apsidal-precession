...
# Uniform in epvals
epvals = npy.random.default_rng(seed=1896).uniform(5e-3, 0.1, N_jobs)

# Log uniform in ompvals
ompvals = npy.power(
    10, npy.random.default_rng(seed=1968).uniform(npy.log10(5e-6), -3, N_jobs)
)

# Uniform in thetap0vals
thetap0vals = npy.random.default_rng(seed=982745).uniform(0, 2 * npy.pi, N_jobs)
...
with open(
    f"results_mup1e-4_tm2pi1e6_n1.55_omalpha0val3.5_random{N_jobs}_nobp_nosec.pkl", "wb"
) as f:
